...
# yawns per minute in Anti-Social condition
(exp(post$b_intercept) * 60) %>%
  median() %>%
  round(2)

# yawns per minute in Pro-Social condition
(exp(post$b_intercept + post$b_condition) * 60) %>%
  median() %>%
  round(2)
```

pMCMC shows that the difference between these is not substantial.

```{r}
sum(post$b_condition > 0) / 4000
```

Let's visualise this lack of a difference on the "yawns per minute" scale.
...
We replace condition with trial order in the model, also as a random effect grouped by individual dogs. This is to see if counterbalancing order has an effect on yawning.
...
Counterbalancing order does not have an effect on yawning.

## 1.4. Condition + Trial model

We include both condition and trial order as additive effects.
...
The 95% CIs for the parameters still cross zero.

## 1.5. Interaction model

Finally, we model the interaction between condition and trial order.
...
As before, the condition parameter has 95% CIs crossing zero, suggesting that there is no difference in yawning rates between conditions. Let's calculate this directly.
...
Estimates are not much different from the zero-inflated model. Again, pMCMC shows that the difference between these is not substantial.
...
We replace condition with trial order in the model.
...
Again, counterbalancing order does not have an effect on yawning.
...
Finally, we fit a Poisson version of the interaction between condition and trial order.
...
Of every model fitted, `m3.4` seems to reign. How does this model do at prediction?
