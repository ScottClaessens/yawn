...
The dogs yawn 0.17 times a minute, on average. Let's split this by ID.

```{r warning=F, message=F, echo=F}
brmstools::forest(m9.1, pars = 'intercept', grouping = 'ID')
```
...
figS5 <-
  ...
  annotation_custom(ggplotGrob(figS5_inset), xmin = 0, xmax = 0.3,
                    ymin = 2, ymax = 4.5)

figS5

# save
ggsave(file = "figures/figS5.pdf", figS5, height = 3, width = 6)
ggsave(file = "figures/figS5.jpg", figS5, height = 3, width = 6)
```
...
How do the effects of condition vary across individual dogs?

```{r echo=F, warning=F, message=F}
brmstools::forest(m9.2, pars = 'hu_condition', grouping = 'ID') +
  geom_vline(xintercept = 0, linetype = "dashed")

brmstools::forest(m9.2, pars = 'condition', grouping = 'ID') +
  geom_vline(xintercept = 0, linetype = "dashed")
```

Condition doesn't seem to have an effect on probability of yawning or yawning rate for any individual dog either.

```{r echo=F}
# cleanup
rm(fig4a_inset, figS5_inset, post)
```
