...
fig2 <-
  ...
  annotation_custom(ggplotGrob(fig2_inset), xmin = 0.3, xmax = 0.55,
                    ymin = 7, ymax = 17)

fig2

# save for publication
ggsave(file = "figures/fig2.pdf", fig2, width = 6, height = 4)
ggsave(file = "figures/fig2.jpg", fig2, width = 6, height = 4)
```
...
fig4 <-
  ...
  theme(axis.line = element_blank(),
        ...
        axis.title.y = element_blank(),
        axis.title.x = element_text(margin = margin(t = 10))) +
  annotation_custom2(ggplotGrob(fig4_insetFamiliar), 
                     data=data.frame(Familiarity="Familiar", Condition = NA, yawns = 1),
                     xmin=0.25, xmax=0.6, ymin=10, ymax=18) +
  annotation_custom2(ggplotGrob(fig4_insetUnfamiliar), 
...
# save for publication
ggsave(fig4, file = "figures/fig4.pdf", width = 6, height = 4)
ggsave(fig4, file = "figures/fig4.jpg", width = 6, height = 4)
```
...
# save for publication
fig1 <- plot_grid(fig1a, NULL, fig1b, ncol = 1, align = 'v', 
                  labels = c("a", "", "b"), rel_heights = c(1, 0.05, 0.82))
ggsave(file = "figures/fig1.pdf", fig1, height = 4.5, width = 6)
ggsave(file = "figures/fig1.jpg", fig1, height = 4.5, width = 6)
```
...
fig3 <- plot_grid(fig3a, NULL, fig3b, nrow = 1, labels = c("a", "", "b"),
                  align = 'h', rel_widths = c(1, 0.1, 1.2))
ggsave(file = "figures/fig3.pdf", fig3, width = 10.1, height = 4.5)
ggsave(file = "figures/fig3.jpg", fig3, width = 10.1, height = 4.5)
```

## 1.12. Condition*Gender interaction
...
```{r warning=F, message=F, echo=F}
  tibble(
    ControlUnfamiliar  = exp(post$b_intercept) * 60,
    YawningUnfamiliar  = exp(post$b_intercept + post$b_condition) * 60,
    ControlFamiliar    = exp(post$b_intercept + post$b_male) * 60,
    YawningFamiliar    = exp(post$b_intercept + post$b_condition 
                             + post$b_male + post$`b_condition:male`) * 60
    ) %>%
  gather(cell, yawns) %>%
  mutate(Condition   = rep(rep(c("Control", "Yawning"), each = 4000), times = 2),
         Gender      = rep(c("Female", "Male"), each = 8000)) %>%
...
```{r}
compare_ic(m1.2, m1.11)
```

This interaction model is worse than its condition-only counterpart.
